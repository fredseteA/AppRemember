                ...
                <button
                  onClick={() => {
                    if (trackingCode.trim()) {
                      onAction(order.id, 'ship', trackingCode);
  ...
  const handleAction = async (orderId, action, trackingCode = null) => {
    try {
      ...
      setQueue(queue.map(order => {
        if (order.id === orderId) {
          const newStatus = action === 'start' ? 'in_production' : 
                           action === 'complete' ? 'produced' : 'shipped';
          return { 
            ...order, 
            status: newStatus,
      ...
      toast.success(
        action === 'start' ? 'Produção iniciada!' :
        action === 'complete' ? 'Produção concluída!' :
        'Pedido enviado!'
      );
    } catch (error) {
